...
+ [优化] 抠图工作台导出的透明图层新增「边缘羽化」滑杆（0–8px，随节点保存），SAM 硬边 mask 转为软 alpha 导出，减轻分层图层的锯齿边。
...
+ [新增] 渠道与模型支持禁用：编辑渠道里可打开「禁用」开关禁用整个渠道（渠道列表显示标识），也可在模型行的能力选择里选「禁用」单独禁用单个模型；禁用后模型不再出现在任何模型选择列表，已选中的禁用模型自动回退到其他可用模型，重新开启后恢复。
+ [修复] 画布节点生成上下文只采用引入该节点的入边资源:此前节点出边指向配置节点时会反向继承该配置的全部上游文本与参考图,导致单独生成图片却混入无关提示词模板,现已移除该反灌逻辑。
+ [新增] 渠道脚本视频生成支持刷新/停止后恢复:脚本创建任务后 `onDelta("任务ID:<id>")` 上报远端任务 ID,画布持久化到节点并在刷新后按 ID 重跑脚本续轮询,中断无需重新生成、不重复计费。
+ [修复] 音频节点默认高度(120)低于缩放通用下限(160)导致一碰缩放手柄就被强制拉高,改为音频节点独立下限 96。
+ [新增] 组节点悬浮工具栏最前新增组底色圆形按钮(10 色色板)与「排列所选节点」下拉(宫格/垂直/水平);框选多节点工具栏同步新增排列菜单,一键重排所选或组内节点位置。
+ [修复] 视频生成节点接入进度显示:调用脚本 `onDelta` 上报与内置轮询估算统一渲染为进度条,修复脚本调用 `onDelta` 报「onDelta is not a function」的问题。
+ [新增] 同步上游 v0.18:视频设置重构为分辨率(480p/720p/1080p)与画幅比例分离计算,时长改 4–30 秒滑杆,支持首尾帧(frames)与全能参考(reference)两种参考模式。
+ [新增] 视频任务 ID 持久化到节点,刷新页面后自动续查任务状态直到成功或失败;节点工具条与信息弹窗展示任务 ID,支持手动刷新。
+ [新增] 调用脚本支持参考视频与参考音频(`videos`/`audios` 变量),默认脚本模板补齐图片质量、背景与参考素材写法,脚本编辑器升级全屏分步引导并支持复制写脚本说明给外部 AI。
+ [新增] 多选节点一键打组/解组,选中多个节点浮现悬浮工具条;组节点作为生成输入时展开引用组内全部资源,左侧元素列表树形展示组层级。
+ [重构] 遮罩编辑改为画布生成遮罩标注图并作为参考图提交(蓝色标注区域提示词驱动),支持「补图重生成」;Gemini 协议提示不支持遮罩。
+ [优化] WebDAV 同步与媒体文件读取支持经本地代理转发;同步不再恢复已删除画布;图片接口返回临时外链时立即下载防止过期;资产下载读取本地文件内容;导入 URL 凭据按 Base URL 自动匹配渠道。
+ [新增] 配置弹窗新增「本地代理」页签，开启后调用脚本请求、图床上传等改为浏览器直传并经本机 canvas-proxy 转发，规避浏览器跨域限制；关闭后恢复走 canvas-agent 中继，行为不变。
+ [修复] 视频时长上限从 20 秒放宽到 30 秒并在秒数选项中新增 30s,修复 30 秒模型(如 SeeDance2.5)在画布选择 30 秒后被钳制成 20 秒导致上游报「时长不可用」的问题。
+ [修复] 免费图床上传移除证书过期的 kingimage，上传链改为 litterbox（失败自动重试）→ uguu → imgBB 兜底，修复参考图上传全链失败导致视频任务无法创建的问题。
+ [新增] 节点插件新增 `fullscreenPanel` 能力:声明后自定义 Panel 在视口级 fixed 容器渲染,支撑插件承载全屏 Modal/弹窗(不受画布缩放变换影响)。
...
+ [修复] 画布渠道直连对话发送 `@` 节点引用时附带 nodeId 映射，第三方模型可精确定位对应节点而非按标签猜测。
+ [新增] 画布渠道直连对话支持多会话管理：切换渠道/模型不丢失历史，新建对话保留旧会话，可在历史页切换和删除。
+ [修复] 画布渠道模型思考程度下拉不再为空，适配 deepseek 等推理模型的 effort 参数并透传到上游 API。
