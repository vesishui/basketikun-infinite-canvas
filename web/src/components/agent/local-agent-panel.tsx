...
export function LocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    // 画布渠道对话：独立于 Codex store 维护，避免与 Codex 线程系统互相覆盖
    const [canvasChatMessages, setCanvasChatMessages] = useState<AgentChatItem[]>([]);
    const canvasChatMessagesRef = useRef<AgentChatItem[]>([]);
    // 是否处于画布渠道模式（驱动 timeline 用 override 渲染）
    const [canvasChannelActive, setCanvasChannelActiveState] = useState(false);
    const canvasChannelActiveRef = useRef(false);
    const setCanvasChat = (next: AgentChatItem[] | ((prev: AgentChatItem[]) => AgentChatItem[])) => {
        // 基于 ref 同步计算（React 的 setState 函数式更新是异步批处理的，不能依赖它同步更新 ref）
        const resolved = typeof next === "function" ? next(canvasChatMessagesRef.current) : next;
        canvasChatMessagesRef.current = resolved;
        setCanvasChatMessages(resolved);
    };
    const setCanvasChannelActive = (active: boolean) => {
    ...
    const resolveCanvasChannelModel = () => {
        ...
        const resolvedModel =
            (modelName && ch.models.some((m) => m.name === modelName) ? modelName : null) ||
            ch.models.find((m) => m.capability === "text")?.name ||
            ch.models[0]?.name ||
            "";
        if (!resolvedModel || !ch.baseUrl || !ch.apiKey) return null;
        return { model: resolvedModel, baseUrl: ch.baseUrl, apiKey: ch.apiKey, displayName: `${resolvedModel}（画布渠道）` };
    };
    ...
    const canvasChannelModel = useMemo(() => {
        ...
        if (!channels.length) return null;
        const textModel = canvasConfig.textModel || "";
        const [channelId, modelName] = textModel.split("::");
        const ch = channels.find((item) => item.id === channelId) || channels[0];
        const resolvedModel =
            (modelName && ch.models.some((m) => m.name === modelName) ? modelName : null) ||
            ...
            "";
        if (!resolvedModel || !ch.baseUrl || !ch.apiKey) return null;
        const displayName = `${resolvedModel}（画布渠道）`;
        return {
            model: resolvedModel,
            baseUrl: ch.baseUrl,
            apiKey: ch.apiKey,
    ...
    const applyConversationState = useCallback((next: AgentConversationState, force = false) => {
        ...
        // 画布渠道模式：只同步 conversation 状态（保证可发送），跳过线程切换/清空，避免覆盖画布直连对话
        if (!canvasChannelActiveRef.current && (conversationChanged || next.threadId !== current.activeThreadId)) {
    ...
    useEffect(() => {
        ...
        source.addEventListener("hello", (event) => {
            ...
            else if (!canvasChannelActiveRef.current) applyWorkspaceChange({ activeThreadId: nextThreadId });
            const current = useAgentStore.getState();
        ...
        source.addEventListener("workspace_changed", (event) => {
            ...
            if (!data) return;
            enqueueEvent(() => {
                // 画布渠道模式：绕过 Codex 线程系统，跳过线程切换/重载，避免覆盖画布直连对话
                if (canvasChannelActiveRef.current) return;
    ...
    const sendPrompt = async () => {
        ...
        const isCanvasSend = Boolean(canvasChannelModel && model === canvasChannelModel.model);
        // 画布渠道的错误提示要进独立对话列表（store 的 error 在画布模式下不渲染）
        const pushCanvasError = (title: string, text: string) => {
            if (isCanvasSend) setCanvasChat((prev) => [...prev, { id: createId(), itemId: "synthetic:error", clientMessageId: "", threadId: "", turnId: "", role: "error", title, text } as AgentChatItem]);
            else addMessage({ role: "error", title, text });
        };
        ...
        if (canvasReferences.some((item) => item.kind === "image")) {
            ...
            try {
                ...
                pushCanvasError(rt("canvasImageReadFailed"), error instanceof Error ? error.message : rt("canvasImageReadFailed"));
                return;
            }
        }
        ...
        if (requestFiles.length > MAX_ATTACHMENTS) {
            ...
            pushCanvasError(rt("tooManyImages"), rt("imageCountLimit", { count: MAX_ATTACHMENTS }));
            return;
        }
        if (attachmentPayloadBytes(requestFiles) > MAX_ATTACHMENT_PAYLOAD_BYTES) {
            setAgentState({ sending: false, activity: rt("imageTooLarge") });
            pushCanvasError(rt("imageTooLarge"), rt("imagePayloadTooLarge"));
            return;
        }
        ...
        // 注意：不能用 addMessage 添加画布用户消息 —— scopeChatItem 会把 user 消息固定成 id `local:pending:synthetic:user`，
        // 第二次发送时 upsertAgentMessage 按 id 命中会直接覆盖第一条用户消息（"我发的消息被覆盖"的根因）。
        // 这里用本次唯一的 messageId 直接写入，保证每次用户消息 id 唯一、顺序正确。
        const userMessageItem: AgentChatItem = { id: messageId, itemId: "synthetic:user", clientMessageId: messageId, threadId: "", turnId: "", role: "user", text: userText, attachments: files, canvasReferences: messageReferences, skill: messageSkill };
        if (isCanvasSend) {
            // 画布渠道：写入独立对话列表（不碰 Codex store）
            setCanvasChat((prev) => upsertAgentMessage(prev, userMessageItem));
        } else {
            setAgentState({ messages: upsertAgentMessage(useAgentStore.getState().messages, userMessageItem) });
        }
        // 画布渠道模型：直连第三方中转对话（绕过 Codex），画布配置改了自动跟随
        if (isCanvasSend) {
            ...
            try {
                ...
                const data = (await res.json().catch(() => ({}))) as { ok?: boolean; content?: string; error?: string; toolCalls?: Array<{ name: string; ok: boolean; summary?: string; error?: string }> };
                if (!data.ok) throw new Error(data.error || "上游返回异常");
                // 记录工具执行日志（画布操作等）
                if (data.toolCalls?.length) {
                    for (const exec of data.toolCalls) {
                        const label = exec.ok ? `画布 · ${exec.name}` : `画布 · ${exec.name}（失败）`;
                        addEventLog(label, exec.summary || exec.error || "");
                    }
                }
                const content = data.content || "（上游未返回内容，请稍后重试）";
                // 更新独立历史供下一轮使用
                directChatHistoryRef.current = [...history, { role: "assistant" as const, content }];
                // 移除旧占位并追加最终消息
                setCanvasChat((prev) => [...prev.filter((item) => item.id !== assistantId), { id: assistantId, itemId: "synthetic:assistant", clientMessageId: assistantId, threadId: "", turnId: "", role: "assistant", title: canvasChannelModel!.displayName, text: content }]);
                saveCanvasSession(canvasChatMessagesRef.current);
            } catch (error) {
                setCanvasChat((prev) => prev.filter((item) => item.id !== assistantId));
                saveCanvasSession(canvasChatMessagesRef.current);
                setAgentState({ sending: false, activity: rt("sendFailed") });
                pushCanvasError(rt("sendFailed"), error instanceof Error ? error.message : String(error));
    ...
    const startNewThread = async () => {
        ...
        if (current.sending || current.waiting || current.loadingThreads || ["preparing", "running"].includes(current.conversation.status)) return;
    ...
    const resumeThread = async (threadId: string) => {
        ...
        const currentCodex = useAgentStore.getState();
        if (currentCodex.sending || currentCodex.waiting || currentCodex.loadingThreads || ["preparing", "running"].includes(currentCodex.conversation.status)) return;
        const operation = beginThreadOperation();
        // 画布模式下点击 Codex 历史记录：先切回 Codex 模型，否则会一直显示画布对话
        if (canvasChannelActiveRef.current) {
            const canvasModel = resolveCanvasChannelModel();
            const store = useAgentStore.getState();
            const codexModel = (store.models.find((m) => !canvasModel || m.model !== canvasModel.model) || store.models[0])?.model;
            if (codexModel) {
                localStorage.setItem("canvas-agent-model", codexModel);
                setCanvasChannelActive(false);
                setAgentState({ model: codexModel });
            }
        }
        try {
    ...
    const content = (
        <>
            ...
            {activeTab === "setup" ? (
                ...
                <>
                    <AgentChatTimeline theme={theme} pendingTool={pendingTool} pendingApprovals={pendingApprovals} sending={sending} waiting={waiting} messagesOverride={canvasChannelActive ? canvasChatMessages : undefined} onRejectTool={rejectPendingTool} onApproveTool={approvePendingTool} onApprovalDecision={decideApproval} />
