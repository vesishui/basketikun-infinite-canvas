...
export function AgentChatTimeline({
    ...
    sending: boolean;
    waiting: boolean;
    /** 画布渠道模式：传入独立渲染的消息列表（不依赖 Codex store），不传则用 store 的 messages */
    messagesOverride?: AgentChatItem[];
    onRejectTool: () => void;
    onApproveTool: () => void;
    onApprovalDecision: (approval: AgentPendingApproval, decision: "accept" | "acceptForSession" | "decline") => void;
}) {
    const { t } = useTranslation();
    const storeMessages = useAgentStore((state) => state.messages);
    ...
    // 画布渠道（有 override）时不显示 Codex bootstrap 状态
    const showBootstrap = Boolean(!messagesOverride && bootstrapStatus && !messages.some((message) => message.role === "user" || message.role === "assistant"));
