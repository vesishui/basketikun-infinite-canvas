...
import { boolConfig, buildApiUrl, isLocalProxyEnabled, modelOptionName, resolveModelRequestConfig, resolveModelScript, withLocalProxy, type AiConfig } from "@/stores/use-config-store";
...
type VideoMediaOptions = RequestOptions & { videos?: ReferenceVideo[]; audios?: ReferenceAudio[]; videoTaskId?: string };
...
/** 视频参考素材：图片 + 音频（如配音、配乐），音频传给脚本的 audios 变量 */
export type VideoReferenceInput = { url?: string; dataUrl?: string };
...
async function createPluginVideoTask(config: AiConfig, model: string, script: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    // 恢复模式（videoTaskId）只需按任务 ID 轮询，不需要参考素材
    const resume = Boolean(options?.videoTaskId);
    const refs = resume ? [] : await Promise.all(references.map((image) => imageToDataUrl(image)));
    const videos = resume ? [] : await Promise.all((options?.videos || []).map((video) => referenceMediaToFile(video, "ref.mp4", "invalidReferenceVideo", options)));
    const audios = resume ? [] : await Promise.all((options?.audios || []).map((audio) => referenceMediaToFile(audio, "ref.mp3", "invalidReferenceAudio", options)));
    const result = videoPluginResult(
        await runModelPlugin({
            ...
            params: {
                ...
                videoTaskId: options?.videoTaskId,
            },
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    try {
        ...
        const created = unwrapVideoResponse((await relayOpenAiRequest({
            ...
            path: "/videos",
            kind: "form",
            body: isLocalProxyEnabled() ? body : await formDataToRelayBody(body),
            signal: options?.signal,
        })) as ApiVideoResponse);
...
/** 浏览器 FormData → relay 需要的 {fields, files:[{name,filename,dataUrl}]}(代理关闭时走中继)。 */
async function formDataToRelayBody(form: FormData) {
    const fields: Record<string, string> = {};
    const files: Array<{ name: string; filename?: string; dataUrl: string }> = [];
    for (const [key, value] of form.entries()) {
        if (value instanceof Blob) {
            files.push({ name: key, filename: value instanceof File && value.name ? value.name : "ref.png", dataUrl: await readFileAsDataUrl(value) });
        } else {
...
async function videoResultFromUrl(url: string, options?: RequestOptions): Promise<VideoGenerationResult> {
    try {
        ...
        if (isLocalProxyEnabled()) {
            blob = (await axios.get<Blob>(withLocalProxy(url), { responseType: "blob", signal: options?.signal })).data;
        } else {
            blob = (await relayOpenAiRequest({ baseUrl: "", apiKey: "", method: "GET", path: url, kind: "blob", signal: options?.signal })) as Blob;
        }
        await assertVideoBlob(blob);
        return { blob };    } catch (error) {
